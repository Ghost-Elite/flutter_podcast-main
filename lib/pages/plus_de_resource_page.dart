...
class PlusDeResourcesPage extends StatefulWidget {
  const PlusDeResourcesPage({Key? key}) : super(key: key);

  @override
  State<PlusDeResourcesPage> createState() => _PlusDeResourcesPageState();
}

class _PlusDeResourcesPageState extends State<PlusDeResourcesPage> {
  ...
  void _firstLoad() async {
    try {
      final url = Uri.https(baseUrls, "/api/resources");
      final res = await http.get(url);

      if (res.statusCode == 200) {
        news = Resources.fromJson(json.decode(res.body));
        setState(() {
          _posts = news!.data;
          lien = news!.links;
                  ...
                  onTap: () {
                    Navigator.of(context)
                      ...
                      return ViewResourcePage(
                        images: _posts[index].image,
                        content: _posts[index].description,
                        urlPdf: _posts[index].file,
                      );
                    }));
                  },
                  child: Stack(
